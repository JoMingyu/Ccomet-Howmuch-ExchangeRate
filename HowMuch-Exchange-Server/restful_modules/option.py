...
class Option(Resource):
    ...
    def get(self):
        ...
        if self.row_exists(id, src_nation, dst_nation):
            rows = self.db.execute(query_formats.option_select_format % (id, src_nation, dst_nation))
    ...
    def post(self):
        ...
        if option_name == 'percentage':
            ...
            if self.row_exists(id, src_nation, dst_nation):
                ...
                self.db.execute(query_formats.percentage_update_format % (fall_percentage, rise_percentage, percentage_criteria, id, src_nation, dst_nation))
                return '', 201
            else:
                # row가 없는 경우
                self.db.execute(query_formats.percentage_insert_format % (id, src_nation, dst_nation, fall_percentage, rise_percentage, percentage_criteria))
                return '', 201
            ...
            if self.row_exists(id, src_nation, dst_nation):
                self.db.execute(query_formats.fixed_value_update_format % (fixed_value_lower_limit, fixed_value_upper_limit, id, src_nation, dst_nation))
                return '', 201
            else:
                self.db.execute(query_formats.fixed_value_insert_format % (id, src_nation, dst_nation, fixed_value_lower_limit, fixed_value_upper_limit))
                return '', 201
            ...
            if self.row_exists(id, src_nation, dst_nation):
                self.db.execute(query_formats.boolean_options_update_format % (every_change, every_rise, every_fall, id, src_nation, dst_nation))
                return '', 201
            else:
                self.db.execute(query_formats.boolean_options_insert_format % (id, src_nation, dst_nation, every_change, every_rise, every_fall))
                return '', 201
